...
## Overview
- HR employee management frontend (Russian UI) on React 19 + TypeScript + Vite; uses MUI v7, Zustand, Axios, React Hook Form + Yup, notistack. Theme colors primary #F54264, secondary #FC8C1E (see [src/app/providers/AppProviders.tsx](src/app/providers/AppProviders.tsx)).
- Auth is SMS code → JWT (access/refresh) stored in localStorage; axios interceptors auto-refresh tokens with a queued retry flow (prevents duplicate refresh requests).
- Architecture docs: [INDEX.md](INDEX.md) maps all feature docs; [ERROR_HANDLING.md](ERROR_HANDLING.md) and per-feature ZD*.md files detail specific patterns.
...
- Token refresh queue: when 401 occurs, first request triggers refresh while subsequent requests wait in queue; all retry with new token after successful refresh (see `isRefreshing` + `failedQueue` in [src/shared/api/http.ts](src/shared/api/http.ts)).
...
- User feedback via notistack snackbars; import `useSnackbar` from 'notistack' and call `enqueueSnackbar(message, { variant: 'success' | 'error' })` for notifications.
...
- Dev proxy: `/api` → https://api.andromedaedu.kz (see [vite.config.ts](vite.config.ts)); avoid bypassing the shared http client/stores.
