...
- `entities/` - Domain models (e.g., `employee/`). Contains types, API calls, and Zustand store. **Single responsibility per entity**.
- `features/` - Feature-specific components (e.g., `employee-create/`). Encapsulates business logic tied to one feature.
- `pages/` - Page-level routes. Compose entities and features, handle layout and navigation.
- `shared/` - Reusable across project: API client, hooks, types, UI utilities.
...
2. **Entity API** (`entities/{name}/api.ts`): Normalize backend responses (handles varying response shapes like `content` vs `items`, `total` vs `totalElements`)
3. **Entity Store** (`entities/{name}/store.ts`): Zustand store manages state + side effects. **Actions fetch AND update state.**
4. **Page Component** (`pages/{name}/`): Consumes store, renders UI, calls handlers on user interaction
